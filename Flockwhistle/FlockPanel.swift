...
enum FlockPanelSession {
    private static let addressKey = "flockwhistle.panel.resume.address"
    private static let stampKey = "flockwhistle.panel.resume.stamp"
...
enum FlockPanelCookies {
    private static let key = "flockwhistle.panel.cookies"
...
struct FlockWebPanel: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        func watchAddress(of webView: WKWebView) {
            ...
            urlObservation = webView.observe(\.url, options: [.new]) { [weak self] webView, _ in
                ...
                FlockPanelSession.remember(webView.url, trackerHost: self.trackerHost)
            }
        }

        func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
            FlockPanelSession.remember(webView.url, trackerHost: trackerHost)
            fire()
        }

        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            FlockPanelSession.remember(webView.url, trackerHost: trackerHost)
            FlockPanelCookies.snapshot()
        }
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        FlockPanelCookies.restore { [weak webView] in
...
struct FlockPrivacyRow: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showPolicy) {
            FlockWebPanel(urlString: "https://valleywinemakerlegacy.org/click.php")
        }
    }
}
